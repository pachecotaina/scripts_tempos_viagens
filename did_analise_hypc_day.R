...
att_summary_table_name <- "/att_log_summary_table_hypc_day.csv"
aggte_combined_summary_name <-  "/aggte_log_combined_summary_hypc_day.csv"
...
for (j in seq_along(control_type)) {
  for (i in seq_along(vec_dep_hours)) {
    ...
    tryCatch({
      ...
      saveRDS(att, paste0(res_folder, "/att_hour_vol_dia.rds"))
      ...
      ggsave(
        paste0(fig_folder, "/ES_per_group_hour_vol_dia_", control_type[j], ".png"), 
        p_es_group, 
        width = 10, 
        height = 6.25)
      ...
      if (length(sig_groups) > 0) {
        ...
        p_groups <- att_filtered %>%
          ...
          mutate(
            ...
            mês = as.factor(time),
            group = paste0("Radares com mudança de velocidade no mês ", sprintf("%02d", group), "/2015")
          ) %>%
        ...
        ggsave(
          paste0(fig_folder, "/siggroups_hour_vol_dia_", control_type[j], ".png"), 
          p_groups_2col, 
          width = 14, 
          height = 8)
        ...
        ggsave(
          paste0(fig_folder, "/siggroups_hour_vol_dia_", control_type[j], "_1col.png"), 
          p_groups_1col, 
          width = 20, 
          height = 27)
        ...
        ggsave(
          paste0(fig_folder, "/eventstudy_hour_vol_dia_", control_type[j], ".png"), 
          p_es, 
          width = 10, 
          height = 6.25)
...
ggsave(
  paste0(fig_folder, "/did_att_all_vol_dia.png"), 
  plot = last_plot(), 
  width = 14, 
  height = 8)
