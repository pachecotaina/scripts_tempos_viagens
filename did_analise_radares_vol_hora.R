...
# Files
all_files <- list.files(folder_path, pattern = "\\d{8}_60\\.parquet$", full.names = TRUE)
valid_files <- all_files %>%
  keep(~ {
    date_part <- str_extract(basename(.x), "^\\d{8}")
...
# Initialize writers for each hour
hour_writers <- setNames(vector("list", 24), str_pad(0:23, 2, "left", "0"))

# Create an empty list for each hour
hour_data <- setNames(vector("list", 24), str_pad(0:23, 2, "left", "0"))

# Process each file and accumulate by hour
walk(valid_files, function(path) {
  ...
  split(df, df$hora) %>% walk(function(df_h) {
    h <- unique(df_h$hora)
    hour_data[[h]] <<- bind_rows(hour_data[[h]], df_h)
  })
})

# Now write each hour to a separate parquet file
walk(names(hour_data), function(h) {
  df_h <- hour_data[[h]]
  if (nrow(df_h) > 0) {
    write_parquet(df_h, file.path(output_dir, glue::glue("radares_hora_{h}.parquet")))
  }
})
...
for (j in seq_along(control_type)) {
  for (i in seq_along(vec_dep_hours)) {
    ...
    tryCatch({
      ...
      saveRDS(att, paste0("results_vol_hour/att_hour_", vec_dep_hours[i], ".rds"))
      ...
      ggsave(
        paste0("figures_vol_hour/ES_per_group_hour_", vec_dep_hours[i], "_", control_type[j], ".png"), 
        p_es_group, 
        width = 10, 
        height = 6.25)
      ...
      if (length(sig_groups) > 0) {
        ...
        ggsave(
          paste0("figures_vol_hour/siggroups_hour_", vec_dep_hours[i], "_", control_type[j], ".png"), 
          p_groups_2col, 
          width = 14, 
          height = 8)
        ...
        ggsave(
          paste0("figures_vol_hour/siggroups_hour_", vec_dep_hours[i], "_", control_type[j], "_1col.png"), 
          p_groups_1col, 
          width = 20, 
          height = 27)
        ...
        ggsave(
          paste0("figures_vol_hour/eventstudy_hour_", vec_dep_hours[i], "_", control_type[j], ".png"), 
          p_es, 
          width = 10, 
          height = 6.25)
...
ggsave(
  paste0("figures_vol_hour/did_att_all_hours.png"), 
  plot = last_plot(), 
  width = 14, 
  height = 8)
...
ggsave(
  paste0("figures_vol_hour/did_att_all_hours_nevertreated.png"), 
  plot = last_plot(), 
  width = 11, 
  height = 5)
...
ggsave(
  paste0("figures_vol_hour/did_att_all_hours_3groups.png"), 
  plot = last_plot(), 
  width = 14, 
  height = 8)

write.csv(att_summary_table, "results_vol_hour/att_summary_table.csv", row.names = FALSE)
write.csv(aggte_combined_summary, "results_vol_hour/aggte_combined_summary.csv", row.names = FALSE)
