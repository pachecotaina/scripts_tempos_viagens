...
vec_ids <- unique(df_result_fhp5$id)
# ############################################################################ #
####              OPEN PARQUET FILE WITH DATA EVERY HOUR                    ####
# ############################################################################ #
# Create output folder if needed
output_dir <- "data/intermediate/radares_fhp5min/"
if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)

# Ensure hora is 2-digit character
df_result_fhp5 <- df_result_fhp5 %>%
  mutate(hora = str_pad(hora, 2, "left", "0"))

# Split by hour and save
split(df_result_fhp5, df_result_fhp5$hora) %>% walk2(
  names(.),
  .,
...
plot_y_lab <-"ATT (Δ fator hora pico - 5 minutos)"
plot_title <- "Efeitos por grupo de tratamento - fator hora pico - 5 minutos"
plot_title_dyn <- "Efeito dinâmico - fator hora pico - 5 minutos"

att_summary_table_name <- "/att_log_summary_table_fhp5min_only07.csv"
aggte_combined_summary_name <-  "/aggte_log_combined_summary_fhp5min_only07.csv"
