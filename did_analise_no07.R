...
for (j in seq_along(control_type)) {
  for (i in seq_along(vec_dep_hours)) {
    ...
    tryCatch({
      ...
      ggsave(
        paste0("figures_no07/ES_per_group_hour_", sprintf("%02d", vec_dep_hours[i]), "_", control_type[j], ".png"), 
        p_es_group, 
        width = 10, 
        height = 6.25)
      ...
      if (length(sig_groups) > 0) {
        ...
        ggsave(
          paste0("figures_no07/siggroups_hour_", sprintf("%02d", vec_dep_hours[i]), "_", control_type[j], ".png"), 
          p_groups_2col, 
          width = 14, 
          height = 8)
        ...
        ggsave(
          paste0("figures_no07/siggroups_hour_", sprintf("%02d", vec_dep_hours[i]), "_", control_type[j], "_1col.png"), 
          p_groups_1col, 
          width = 20, 
          height = 27)
        ...
        ggsave(
          paste0("figures_no07/eventstudy_hour_", sprintf("%02d", vec_dep_hours[i]), "_", control_type[j], ".png"), 
          p_es, 
          width = 10, 
          height = 6.25)
...
ggsave(
  paste0("figures_no07/did_att_all_hours.png"), 
  plot = last_plot(), 
  width = 14, 
  height = 8)
...
ggsave(
  paste0("figures_no07/did_att_all_hours_nevertreated.png"), 
  plot = last_plot(), 
  width = 11, 
  height = 5)
...
ggsave(
  paste0("figures_no07/did_att_all_hours_3groups.png"), 
  plot = last_plot(), 
  width = 14, 
  height = 8)

write.csv(att_summary_table, "results/att_no07_summary_table.csv", row.names = FALSE)
write.csv(aggte_combined_summary, "results/aggteno07__combined_summary.csv", row.names = FALSE)
