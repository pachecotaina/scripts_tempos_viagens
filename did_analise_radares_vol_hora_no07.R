...
####                              SETUP                                     ####
# ############################################################################ #
res_folder <- "results_vol_hour_no07"
fig_folder <- "figures_vol_hour_no07"
...
att_summary_table_name <- "/att_log_summary_table_vol_hour_no07.csv"
aggte_combined_summary_name <-  "/aggte_log_combined_summary_vol_hour_no07.csv"
  ...
  for (i in seq_along(vec_dep_hours)) {
    ...
    tryCatch({
      ...
      df_hour <- list_hourly_data[[i]] %>% 
        filter(month_treat %notin% c(6,7))
