...
vec_ids <- unique(df_result_hour$id)
# ############################################################################ #
####              OPEN PARQUET FILE WITH DATA EVERY HOUR                    ####
# ############################################################################ #
# Create output folder if needed
output_dir <- "data/intermediate/radares_hypc_hour/"
...
split(df_result_hour, df_result_hour$hora) %>% walk2(
  names(.),
  .,
...
plot_y_lab <-"ATT (Δ probabilidade de supersaturação)"
plot_title <- "Efeitos por grupo de tratamento - probabilidade de supersaturação por dia"
plot_title_dyn <- "Efeito dinâmico - probabilidade de supersaturação por dia"

att_summary_table_name <- "/att_log_summary_table_hypc_hour_only07.csv"
aggte_combined_summary_name <-  "/aggte_log_combined_summary_hypc_hour_only07.csv"
